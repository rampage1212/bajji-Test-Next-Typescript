...
const JobStoriesComponent = () => {
  ...
  useEffect(() => {
    const loadStories = async () => {
      try {
        const data = await fetchStories(16, StoryTypes.job);
        setStories(data);
      } catch (err) {
  ...
  return (
    <div className="w-full px-6 pb-10">
      <p className="pt-10 text-5xl font-bold leading-[52px]">Jobs HN</p>
      {stories.length > 0 ? (
        <>
...
export default JobStoriesComponent;
